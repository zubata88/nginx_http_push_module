...
static ngx_http_push_msg_t * ngx_http_push_find_message_locked(ngx_http_push_channel_t *channel, ngx_http_request_t *r, ngx_int_t *status) {
	...
	if(time <= msg->id.time) { //that's an empty check (Sentinel's values are zero)
		if(time == msg->id.time) {
	...
	while(cur!=sentinel) {
		...
		else if(time == msg->id.time) {
			if(tag<0) { tag = ngx_http_push_subscriber_get_etag_int(r); }
			while (tag >= msg->id.tag  && time == msg->id.time && ngx_queue_next(cur)!=sentinel) {
				cur=ngx_queue_next(cur);
				msg = ngx_queue_data(cur, ngx_http_push_msg_t, queue);
			}
			if(time == msg->id.time && tag < msg->id.tag) {
				*status = NGX_HTTP_PUSH_MESSAGE_FOUND;
				return msg;
			}
			continue;
...
static ngx_str_t * ngx_http_push_get_channel_id(ngx_http_request_t *r, ngx_http_push_loc_conf_t *cf, ngx_http_push_channel_id_t **next_channel_id) {
	...
	if(!ngx_queue_empty(&cf->channel_id_sentinel)) {
		...
		if(!cf->multiplex_channels) {
			//no multiplexing. 
			//use only the first channel id.
			chan_id = (ngx_http_push_channel_id_t *)ngx_queue_head(&cf->channel_id_sentinel);
			*next_channel_id=NULL;
		}
		else {
			//channel is multiplexed.
			//we'll need to go through all the ids, going over one per this function call.
			if(*next_channel_id==NULL) { //reset!
				*next_channel_id = (ngx_http_push_channel_id_t *)ngx_queue_head(&cf->channel_id_sentinel);
			}
			chan_id = *next_channel_id;
			*next_channel_id = (ngx_http_push_channel_id_t *) (ngx_queue_next(&(*next_channel_id)->queue)==&cf->channel_id_sentinel ? NULL : ngx_queue_next(&(*next_channel_id)->queue));
		}
		ngx_http_complex_value(r, chan_id->ccv->complex_value, &partial_id);
	}
...
static ngx_int_t ngx_http_push_set_etag(ngx_str_t *etag, time_t basetime, ngx_http_push_loc_conf_t *cf, ngx_http_push_channel_scratch_t scratch[], ngx_int_t finish_later_channel_number) {
	ngx_int_t                       channels = cf->multiplex_channels ? 1 : cf->channel_id_count;
	ngx_int_t                       i;
	u_char                          *cur = etag->data;
	for(i=0; i<channels; i++) {
		if(finish_later_channel_number != i) {
			cur = ngx_sprintf(cur, "|%ui:%ui", scratch[i].msg->id.time - basetime, scratch[i].msg->id.tag);
		}
		else {
			ngx_memcpy(cur, "|%ui:%ui", 8);
			cur+= 8;
		}
	}
	etag->len = cur - etag->data;
	return NGX_OK;
}
...
static ngx_http_push_channel_scratch_t *ngx_http_init_channel_scratch(ngx_http_push_loc_conf_t *cf) {
	if( cf->channel_scratch == NULL &&
	   (cf->channel_scratch = ngx_palloc(ngx_http_push_pool, sizeof(ngx_http_push_channel_scratch_t) * cf->channel_id_count)) == NULL)
	{
		return NULL;
	}
	return cf->channel_scratch;
}
...
static ngx_int_t ngx_http_push_handle_subscriber_channel(ngx_http_request_t *r, ngx_http_push_loc_conf_t *cf, ngx_http_push_channel_t *channel, ngx_http_push_channel_scratch_t *scratch) {
	...
	ngx_shmtx_lock(&ngx_http_push_shpool->mutex);
	msg = ngx_http_push_find_message_locked(channel, r, &scratch->msg_search_outcome); 
...
static ngx_int_t ngx_http_push_subscriber_handler(ngx_http_request_t *r) {
	...
	do {
		...
		if((id=ngx_http_push_get_channel_id(r, cf, &next_channel_id))!=NULL) {
			...
			ngx_shmtx_unlock(&ngx_http_push_shpool->mutex);
			scratch_cur->response_code = ngx_http_push_handle_subscriber_channel(r, cf, channel, scratch_cur);
			...
			if(scratch_cur->msg_search_outcome < scratch_chosen->msg_search_outcome) {
				...
				if(scratch_cur->msg_search_outcome == NGX_HTTP_PUSH_MESSAGE_FOUND) {
					...
					if(msg_found->id.time < msg_chosen->id.time ||
					  (msg_found->id.time == msg_chosen->id.time && msg_found->id.tag < msg_chosen->id.tag)) { //assumes unique message_tags per second
						scratch_chosen = scratch_cur;
					}
				}
			}
	...
	switch(scratch_chosen->msg_search_outcome) {
		...
		time_t                      basetime = scratch_chosen->msg->id.time;
		...
		case NGX_HTTP_PUSH_MESSAGE_EXPECTED:
			...
			switch(cf->subscriber_poll_mechanism) {
				...
				ngx_http_push_subscriber_cleanup_t *clndata, *clndata_prev, *clndata_first;
				...
				case NGX_HTTP_PUSH_MECHANISM_LONGPOLL:
					...
					for(i=0; i < num_channels; i++) {
						if(scratch[i].msg_search_outcome==NGX_HTTP_PUSH_MESSAGE_EXPECTED) {
							...
							subscriber->message_tag.data = (u_char *)(subscriber+1);
		...
		case NGX_HTTP_PUSH_MESSAGE_FOUND:
			...
			if((etag = ngx_pcalloc(r->pool, sizeof(*etag) + ngx_http_push_get_etag_length(cf)))==NULL) {
				//oh, nevermind...
				ngx_shmtx_unlock(&ngx_http_push_shpool->mutex);
...
static void ngx_http_push_publisher_body_handler(ngx_http_request_t * r) {
	...
	switch(method) {
		...
		case NGX_HTTP_POST:
			...
			msg->id.tag=ngx_http_push_generate_message_tag_locked(msg->id.time);
...
static ngx_uint_t ngx_http_push_generate_message_tag_locked(time_t t) {
	ngx_http_push_msg_id_t         *lastid = &((ngx_http_push_shm_data_t *) ngx_http_push_shm_zone->data)->tag_counter;
	if(lastid->time == t) {
		return ++(lastid->tag);
	}
	else {
...
static ngx_int_t ngx_http_push_respond_to_subscribers(ngx_http_push_channel_t *channel, ngx_http_push_subscriber_t *sentinel, ngx_http_push_msg_t *msg, ngx_int_t status_code, const ngx_str_t *status_line) {
	...
	if(msg!=NULL) {
		...
		//etag
		//NGX_HTTP_PUSH_MAKE_ETAG(msg->message_tag, etag, ngx_pcalloc, ngx_http_push_pool); //NO!
		if(etag==NULL) {
			//oh, nevermind...
			ngx_shmtx_unlock(&shpool->mutex);
			return NGX_ERROR;
		...
		last_modified_time = msg->id.time;
...
static void ngx_http_push_subscriber_cleanup(ngx_http_push_subscriber_cleanup_t *data) {
	...
	if(data->next!=NULL) {
		...
		ngx_http_push_subscriber_cleanup(data->next);
	}
}
